...
var getResponseFromUrl = function (endpoint, urlObj, swaggerSpec, options, update, apiPredefinedConfig, callback) {
  let apiPath = getApiPath(urlObj, swaggerSpec);
    ...
    if (error != null) {
      ...
      if (body && body != '' && response.headers['content-type'].indexOf('application/json') > -1) {
        ...
        if (apiPredefinedConfig && apiPredefinedConfig.optionalResponse && apiPredefinedConfig.optionalResponse[response.statusCode]) {          
          apiPredefinedConfig.optionalResponse[response.statusCode].forEach(function (optional) {            
            if (schemaObj.items && schemaObj.items.required && schemaObj.items.required.indexOf(optional) > -1) schemaObj.items.required.splice(schemaObj.items.required.indexOf(optional), 1);
